# Results Directory Structure

This directory contains organized benchmark results with auto-incrementing run numbers.

## Directory Layout

```
results/
├── token_reduction/         # Token reduction benchmarks
│   ├── run_001_results.csv
│   ├── run_001_summary.json
│   ├── run_002_results.csv
│   └── ...
├── llm_comprehension/       # LLM comprehension tests
│   ├── run_001_gpt35turbo.json
│   ├── run_002_gpt4o.json
│   └── ...
└── visualizations/          # Generated charts
    ├── token_reduction/     # Token reduction charts
    │   ├── run_001_token_reduction.png
    │   ├── run_001_similarity_tradeoff.png
    │   ├── run_001_per_file_analysis.png
    │   └── run_001_processing_time.png
    └── llm_comprehension/   # LLM comprehension charts
        ├── run_002_comprehension_by_file.png
        ├── run_002_similarity_distribution.png
        ├── run_002_token_savings_tradeoff.png
        ├── run_002_question_type_analysis.png
        └── run_002_summary.json

output/
└── minified/                # Minified markdown files
    ├── technical_doc_syntax_stopwords_simplify.mm
    ├── readme_example_syntax.mm
    └── ...
```

## Run Numbering

- Each benchmark type maintains its own auto-incrementing counter
- Runs are numbered sequentially: `run_001`, `run_002`, etc.
- Filenames include identifiers for the test type or model used

## Token Reduction Benchmarks

Generated by `scripts/benchmark.py`:
- `run_XXX_results.csv` - Detailed per-file, per-strategy results
- `run_XXX_summary.json` - Aggregate statistics

## LLM Comprehension Benchmarks

Generated by `scripts/llm_benchmark.py`:
- `run_XXX_<model>.json` - Full Q&A results with comprehension metrics
- Includes: questions, answers (original vs minified), token savings, similarity scores

## Visualizations

Generated by `scripts/visualize.py` (token reduction) and `scripts/visualize_llm.py` (LLM comprehension):

### Token Reduction Visualizations
- `run_XXX_token_reduction.png` - Bar chart of reduction by strategy
- `run_XXX_similarity_tradeoff.png` - Scatter plot of reduction vs similarity
- `run_XXX_per_file_analysis.png` - Detailed per-file breakdown
- `run_XXX_processing_time.png` - Performance comparison

### LLM Comprehension Visualizations
- `run_XXX_comprehension_by_file.png` - Comprehension % by document
- `run_XXX_similarity_distribution.png` - Histogram of answer similarities
- `run_XXX_token_savings_tradeoff.png` - Token savings vs comprehension
- `run_XXX_question_type_analysis.png` - Success rate by question type
- `run_XXX_summary.json` - Detailed metrics and statistics

## Usage

All scripts automatically use the next available run number:

```bash
# Token reduction benchmark (auto-numbered)
python3 scripts/benchmark.py testdata/samples

# Visualize latest token reduction results
python3 scripts/visualize.py

# LLM comprehension test (auto-numbered)
python3 scripts/llm_benchmark.py testdata/samples --model gpt-4o

# Visualize latest LLM comprehension results
python3 scripts/visualize_llm.py

# Or specify custom paths
python3 scripts/visualize.py --input results/token_reduction/run_001_results.csv
python3 scripts/visualize_llm.py --input results/llm_comprehension/run_002_gpt4o.json

# Minify files with descriptive output names (auto-generated)
python3 src/minimark.py input.md --strategies syntax stopwords
# → output/minified/input_syntax_stopwords.mm
```

## Minified Output Naming

Minified files use descriptive names showing the strategies applied:
- Format: `{filename}_{strategy1}_{strategy2}_{...}.mm`
- Example: `technical_doc_syntax_stopwords_simplify.mm`
- Makes it easy to compare different strategy combinations
